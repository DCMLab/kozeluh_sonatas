...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/kozeluh_sonatas.git
...
* `Leopold Koželuch – Piano Sonatas <https://github.com/DCMLab/kozeluh_sonatas/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first movement of the sonata in C, op. 8 no. 1, has the following files:

-  ``MS3/09op08no1a.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/09op08no1a.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/09op08no1a.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/09op08no1a.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/09op08no1a.tsv')
   notes = ms3.load_tsv('notes/09op08no1a.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/kozeluh_sonatas/issues>`__ and feel free to fork and submit pull requests.
